...
from tests.mocks import BDD_COMPETITIONS
...
class TestIndividualCompetitions:
    """
    ...
    def test_show_competitions(self, client, mock_clubs, mock_competitions):
        ...
        assert len(mock_competitions) == len(BDD_COMPETITIONS)
        for competition in mock_competitions:
            assert f"{competition['name']}".encode() in homepage.data
